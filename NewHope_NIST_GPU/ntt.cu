...
*              polynomials before number theoratic transform
************************************************************/
__device__  uint16_t bitrev_table[NEWHOPE_N] = {
...
__device__ void bitrev_vector(poly* poly)
{
	...
	for (i = 0; i < NEWHOPE_N; i++)
	{
		...
		if (i < r)
		{
			tmp = poly->coeffs[i].threads[X];
			poly->coeffs[i].threads[X] = poly->coeffs[r].threads[X];
			poly->coeffs[r].threads[X] = tmp;
		}
	}
}
...
__device__ void mul_coefficients(poly* poly, const uint16_t* factors)
{
	...
	for (i = 0; i < NEWHOPE_N; i++)
		poly->coeffs[i].threads[X] = montgomery_reduce((poly->coeffs[i].threads[X] * factors[i]));
}
...
*                                       assumed to be in Montgomery domain
**************************************************/
__device__ void ntt(poly* a, const uint16_t* omega)
{
	int X = threadIdx.x + blockIdx.x * blockDim.x;
	...
	for (i = 0; i < 10; i += 2)
	{
		...
		for (start = 0; start < distance; start++)
		{
			...
			{
				W = omega[jTwiddle++];
				temp = a->coeffs[j].threads[X];
				a->coeffs[j].threads[X] = (temp + a->coeffs[j+distance].threads[X]); // Omit reduction (be lazy)
				a->coeffs[j + distance].threads[X] = montgomery_reduce((W * ((uint32_t)temp + 3 * NEWHOPE_Q - a->coeffs[j + distance].threads[X])));
			}
		}
		...
		for (start = 0; start < distance; start++)
		{
			...
			for (j = start; j < NEWHOPE_N - 1; j += 2 * distance)
			{
				W = omega[jTwiddle++];
				temp = a->coeffs[j].threads[X];
				a->coeffs[j].threads[X] = barrett_reduce(temp + a->coeffs[j + distance].threads[X]);
				//a->coeffs[j].threads[X] = (temp + a->coeffs[j + distance].threads[X]) % NEWHOPE_Q;
				a->coeffs[j + distance].threads[X] = montgomery_reduce((W * ((uint32_t)temp + 3 * NEWHOPE_Q - a->coeffs[j + distance].threads[X])));
			}
		}
	}
