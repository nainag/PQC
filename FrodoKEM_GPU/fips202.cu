...
__host__  __device__ static uint64_t load64(const unsigned char *x)
{
...
__device__ static const uint64_t KeccakF_RoundConstants_DEV[NROUNDS] =
{
	(uint64_t)0x0000000000000001ULL,
	(uint64_t)0x0000000000008082ULL,
	(uint64_t)0x800000000000808aULL,
...
__device__ void sha3_keccakf(uint64_t st_io[25])
{
	// variables
	int i, j, r;
	uint64_t t, bc[5];
	...
	for (r = 0; r < NROUNDS; r++) {
		...
		//  Iota
		st[0] ^= KeccakF_RoundConstants_DEV[r];
	}
...
__device__ void sha3_keccakf_Sh(uint64_t *st_io, int threadIdx)
{
	// variables
	int i, j, r;
	uint64_t t, bc[5];
	...
	for (int i = 0; i < 25; i++)
	{
		st[i] = st_io[threadIdx + i * THR_PER_BLK];
	}
	...
	for (int i = 0; i < 25; i++)
	{
		st_io[threadIdx + i * THR_PER_BLK] = st[i];
	}
...
__device__ static void keccak_absorb_DEV(uint64_t *s, unsigned int r, const unsigned char *m, unsigned long long int mlen, unsigned char p)
{
	unsigned long long i;
	unsigned char t[200];
...
__device__ static void keccak_absorb_DEVSH(uint64_t *s, int threadIdx, unsigned int r, const unsigned char *m, unsigned long long int mlen, unsigned char p)
{
	unsigned long long i;

	for (i = 0; i < 25; ++i)
		s[threadIdx + i * THR_PER_BLK] = 0;

	for (i = 0; i < (mlen / 8); ++i)
		s[threadIdx + i * THR_PER_BLK] = ((uint64_t*)m)[i];

	auto kk = (uint8_t*)(s + (threadIdx + i * THR_PER_BLK));
	auto kl = (uint8_t*)(s + (threadIdx + ((r / 8) - 1) * THR_PER_BLK));
...
__device__  static void keccak_squeezeblocks_DEVSH(unsigned char *h, unsigned long long int nblocks, uint64_t *s, int threadIdx, unsigned int r)
{
	unsigned int i;

	while (nblocks > 0)
	{
		//KeccakF1600_StatePermute_DEV(s);
		sha3_keccakf_Sh(s, threadIdx);

		for (i = 0; i < (r >> 3); i++)
		{
			store64(h + 8 * i, s[threadIdx + i * THR_PER_BLK]);
		}
		h += r;
		nblocks--;
	}
...
__device__  static void keccak_squeezeblocks_DEV(unsigned char *h, unsigned long long int nblocks, uint64_t *s, unsigned int r)
{
	unsigned int i;
...
__device__ void shake128_Sh(unsigned char *output, unsigned long long outlen, const unsigned char *input, unsigned long long inlen, uint64_t *s, int threadIdx, int blockId)
{
	...
	keccak_absorb_DEVSH(s, threadIdx, SHAKE128_RATE, input, inlen, 0x1F);
	
	keccak_squeezeblocks_DEVSH(output, nblocks, s, threadIdx, SHAKE128_RATE);
	...
	if (outlen)
	{
		keccak_squeezeblocks_DEVSH(t, 1, s, threadIdx, SHAKE128_RATE);
		for (i = 0; i < outlen; i++)
		{
			output[i] = t[i];
		}
...
__global__ void shake256_kernel(unsigned char *output, unsigned long long outlen, unsigned char *input, unsigned long long inlen)
{
	...
	/* Absorb input */
	keccak_absorb_DEV(s, SHAKE256_RATE, input, inlen, 0x1F);

	/* Squeeze output */
	keccak_squeezeblocks_DEV(output, nblocks, s, SHAKE256_RATE);
	...
	if (outlen)
	{
		keccak_squeezeblocks_DEV(t, 1, s, SHAKE256_RATE);
		for (i = 0; i < outlen; i++)
			output[i] = t[i];
	}
}
...
__global__ void shake256_kernel_single(unsigned char *output, unsigned long long outlen, unsigned char *input, unsigned long long inlen, unsigned char const_byte)
{
	...
	if (seed_id < 230)
	{
		...
		output += (SHAKE256_RATE * seed_id);

		for (i = 0; i < SHAKE256_RATE / 8; i++)
		{
			store64(output + 8 * i, s[i]);
		}
